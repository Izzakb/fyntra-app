...
// 1. DEFINISI INTERFACE (Agar TypeScript tidak error)
...
export default function DashboardHome({
  ...
  balance,
  onUpdate,
  onNavigate, // 2. TERIMA PROP NAVIGASI DI SINI
}: DashboardHomeProps) {
  const [loading, setLoading] = useState(false);
  const [transactions, setTransactions] = useState<any[]>([]);
  const [goals, setGoals] = useState<any[]>([]);

  // State untuk form transaksi tunggal
    ...
    <div className="animate-in fade-in duration-700 space-y-10 pb-20">
      ...
      <div className="bg-white p-10 rounded-[3.5rem] border border-slate-100 shadow-sm">
        <div className="flex justify-between items-center mb-8">
          ...
          {/* 4. TOMBOL NAVIGASI KE TRANSACTIONS */}
          <button
        ...
        <div className="space-y-4">
          ...
          {transactions.length === 0 && (
            <p className="text-center text-slate-400 font-bold text-[10px] uppercase italic py-4">
              Belum ada transaksi.
            </p>
          )}
        </div>
