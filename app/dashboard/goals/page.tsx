  ...
  return (
    <div className="animate-in fade-in duration-700 pb-20 bg-transparent">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {goals.map((g) => {
            ...
            return (
              <div
                key={g.id}
                className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm relative overflow-hidden group transition-all duration-300"
              >
                <div className="flex justify-between items-start mb-6 relative z-10">
                  <div>
                    <h3 className="text-xl font-black italic text-slate-900 dark:text-white uppercase tracking-tighter transition-colors">
                      {g.goal_name}
                    </h3>
                  ...
                  {isAchieved ? (
                    <span className="px-4 py-2 bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 rounded-xl text-[9px] font-black uppercase tracking-widest transition-colors">
                      Achieved 🎯
                    </span>
                  ) : (
                    <button
                      onClick={() => handleDeleteGoal(g.id)}
                      className="opacity-0 group-hover:opacity-100 transition-opacity text-[10px] font-black uppercase text-rose-400 hover:text-rose-600 dark:text-rose-500"
                    >
                      Delete
                    </button>
                  )}
                ...
                <div className="space-y-3 relative z-10">
                  <div className="flex justify-between items-end">
                    ...
                    <p
                      className={`font-black text-lg italic transition-colors ${isAchieved ? "text-emerald-500" : "text-blue-600 dark:text-blue-400"}`}
                    >
                      {progress}%
                    </p>
                  </div>
                  <div className="w-full h-4 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden transition-colors">
                    <div
                      className={`h-full transition-all duration-1000 ${isAchieved ? "bg-emerald-500" : "bg-blue-600 dark:bg-blue-500"}`}
                ...
                {isAchieved && (
                  <div className="absolute -right-10 -bottom-10 w-40 h-40 bg-emerald-500/10 dark:bg-emerald-500/5 rounded-full blur-3xl z-0 pointer-events-none transition-colors"></div>
                )}
              </div>
            );
          })}
        ...
        <div className="bg-white dark:bg-slate-900 p-20 rounded-[3rem] border border-slate-100 dark:border-slate-800 text-center shadow-sm transition-all">
          <p className="text-slate-300 dark:text-slate-700 font-black italic uppercase tracking-widest">
          ...
          <div className="bg-white dark:bg-slate-900 w-full max-w-md p-10 rounded-[3rem] shadow-2xl animate-in zoom-in duration-300 border border-transparent dark:border-slate-800">
            ...
            <form onSubmit={handleAddGoal} className="space-y-6">
              <input
                ...
                className="w-full px-6 py-4 bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-800 rounded-2xl font-bold text-slate-900 dark:text-white outline-none transition-all"
                value={goalName}
              ...
              <div className="flex gap-4 pt-2">
                ...
                <button
                  type="submit"
                  className="flex-1 py-4 bg-slate-900 dark:bg-blue-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-lg active:scale-95"
                >
                  Save Target
                </button>
              </div>
